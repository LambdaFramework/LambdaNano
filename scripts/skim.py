...
class skimmer:
    ...
    def initialization(self, year_):
        if not os.environ['NANOAODTOOLS_BASE']:
            print("ERROR: NANOAODTOOLS_BASE not set"); sys.exit();
            
        theList = map( lambda x: x.split('.')[0] , os.listdir( '%s/scripts/filelists/%s' %( os.environ["NANOAODTOOLS_BASE"] , year_ ) ) )
        ...
        for i in fnames :
            sample_files = open( "%s/scripts/filelists/%s/%s.txt" %( os.environ["NANOAODTOOLS_BASE"] , year_ , i ) , "r" )
...
if __name__ == "__main__" :
    ...
    skim = skimmer( options.dataset , '%s-%s' %(options.outfolder,options.year) )
    skim.initialization(options.year)
