...
class ChargeFlipWeight(Module):
    def __init__(self, cmssw, MCType ='DY', useData = True , CF_cfg = 'ChargeFlip_cfg.py' , collection="Lepton"):
        #self.cmssw = cmssw
        self.MCType = MCType
        self.useData = useData
        cmssw_base = os.getenv('NANOAODTOOLS_BASE')
        #ScaleFactorFile = cmssw_base + '/src/' + CF_cfg
        ScaleFactorFile = cmssw_base + '/python/postprocessing/modules/analysis/ChargeFlips/' + CF_cfg
    ...
    def analyze(self, event):
        ...
        nLep = len(list(leptons))
        genLeptons = Collection(event, "LeptonGen" )
        nGenLep = len(list(genLeptons))
