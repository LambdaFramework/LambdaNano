...
class alias_Producer(Module):
    ...
    def analyze(self, event):
        ...
        if event.nCleanJet>=2 : dijet = jets[cleanjets[0].jetIdx].p4() + jets[cleanjets[1].jetIdx].p4()
        if event.nCleanJet>=2 and event.nLepton>=1 :
            dijetlep1 = dijet + electrons[leptons[0].electronIdx].p4() if abs(leptons[0].pdgId)==11 else dijet + muons[leptons[0].muonIdx].p4()
        if event.nCleanJet>=2 and event.nLepton>=2 :
